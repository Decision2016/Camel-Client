...
void camel_client::signUser(QString username, QString password) {
    ...
    RSA_generate_key_ex(client_key, RSA_KEY_BIT_LENGTH, bignum, NULL);
    BN_free(bignum);
    ...
    while (1) {
        ...
        if (n != -1) {
            ...
            if (statusCode == SERVER_SECOND_CONNECT) {
                aesDecrypt((unsigned char*)&recv_buffer[2], (unsigned char*)buffer, DATA_START);
    ...
    while (true) {
        ...
        if (n != -1) {
            ...
            if (statusCode == SERVER_FIRST_CONNECT && checkSign(&recv_buffer[2]) == 0) {
                ...
                server_key = d2i_RSAPublicKey(nullptr, &rsa_start, RSA_KEY_LENGTH);
                client_socket = clientSocket;
            }
            break;
        }
...
QString camel_client::getDirInfo() {
    ...
    aesEncrypt((unsigned char*) buffer, (unsigned char*)send_buffer, DATA_START);
    send(client_socket, send_buffer, BUFFER_LENGTH, 0);
